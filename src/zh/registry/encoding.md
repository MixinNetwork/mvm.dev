...
之后是每个合约调用的编码，由 合约地址 + 合约输入长度的十六进制 + 合约输入 构成，合约输入由 函数签名的 ABI 编码的前 8 位 + 函数输入的 ABI 编码 构成（[代码示例](https://github.com/MixinNetwork/mvm-contracts/blob/main/contracts/mixin/User.sol#L52)）：
